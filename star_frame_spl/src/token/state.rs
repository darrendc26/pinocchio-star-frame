...
#[derive(
    Debug, Copy, Clone, PartialEq, Eq, Default, Zeroable, CheckedBitPattern, Align1, NoUninit,
)]
#[repr(C, packed)]
...
impl<'a> CanInitAccount<InitMint<'a>> for MintAccount {
    fn init_account<const IF_NEEDED: bool>(
        ...
        let funder = ctx
            .get_funder()
            .ok_or_else(|| return Err(ErrorCode::EmptyFunderCache))?;
    ...
    MintInfo: SingleAccountSet,
{
    fn init_account<const IF_NEEDED: bool>(
        ...
        let funder = ctx.get_funder().ok_or_else(|| {
            return Err(ErrorCode::EmptyFunderCache.into());
        })?;
        self.init_account::<IF_NEEDED>((arg, funder), account_seeds, ctx)
    }
}
