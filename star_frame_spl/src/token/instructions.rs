...
#[cfg_attr(not(test), derive(Copy, InstructionSet))]
#[cfg_attr(test, derive(Copy, Clone, Debug, PartialEq, Eq, InstructionSet))]
#[ix_set(use_repr)]
#[repr(u8)]
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, TypeToIdl)]
...
#[derive(AccountSet)]
pub struct InitializeMintAccounts<'a> {
    pub mint: Account<'a, MaybeUninitialized<Mint>>,
...
// initialize account
/// See [`spl_token_interface::instruction::TokenInstruction::InitializeAccount`].
#[derive(Copy, Clone, InstructionArgs)]
#[type_to_idl(program = crate::token::Token)]
pub struct InitializeAccount;
/// Accounts for the [`InitializeAccount`] instruction.
///
